const FilterItem = () => {
  return (
    <div className="border-solid border-customBlue border-4 p-3 rounded-md flex gap-6 max-lg:flex-col">
        <img src="/images/features.jpeg" alt="Image" className="w-96 object-cover rounded-md max-xl:w-full" />
        <div className="w-full flex gap-10 max-xl:flex-col">
            <div className="flex flex-col gap-3 flex-2">
                <h1 className="text-2xl text-customBlue">
                    Tower Street Apartments
                </h1>
                <span>
                    500m from center
                </span>
                <span className="bg-green-400 font-semibold text-customWhite w-fit p-3 rounded-md">
                    Free Aiport Taxi
                </span>
                <span className="font-bold">
                    Studio Apartment With Air Conditioner
                </span>
                <span>
                    Entire Studio + 1 Bathroom + 21m + 1 Full Bed
                </span>
                <span className="font-bold">
                    Free Cancellation
                </span>
                <span>
                    You can cancel later, so lock in this great price today!
                </span>
            </div>
            <div className="flex flex-col justify-between flex-1 min-w-fit gap-3">
                <div className="flex justify-between">
                    <span className="text-2xl">
                        Excellent
                    </span>
                    <label className="bg-customGold text-customWhite p-3 font-bold rounded-md border-none">
                        8.9
                    </label>
                </div>
                <div className="text-right flex flex-col items-end gap-3">
                    <span className="text-3xl">
                        $123
                    </span>
                    <span className="text-gray-400">
                        Includes taxes and fees
                    </span>
                    <button className="w-full bg-customBlue text-customWHite font-bold p-3 text-customWhite rounded-md hover:bg-opacity-50 duration-200">
                        See Availability
                    </button>
                </div>
            </div>
        </div>
    </div>
  )
}

export default FilterItem;