    ...
    return (
        <div className={`w-full p-3 bg-customWhite border-4 border-solid border-customGold flex max-xl:flex-col max-xl:gap-y-3 items-center justify-around rounded-md ${page === "home" && "absolute -bottom-16 max-xl:-bottom-40"}`}>
            <div className="max-xl:w-full flex items-center gap-5">
                ...
                <input type="text" defaultValue={params?.city} onChange={(event) => setDestination(event.target.value)} placeholder="Where are you going?" className="max-xl:w-full border-solid border-2 outline-none border-gray-400 p-2 rounded-md text-gray-400" />
            </div>
            ...
            <div className="max-xl:w-full max-xl:justify-center flex items-center gap-5">
                <button onClick={() => push(`/hotels?city=${destination}&sort=${sort}&range=${priceRange.from},${Boolean(priceRange.to) ? priceRange.to : 999999999}`)} className="max-xl:w-full bg-customBlue text-white font-bold p-3 rounded-md cursor-pointer hover:bg-opacity-50 duration-200">
                    Search
                </button>
            </div>
        </div>
